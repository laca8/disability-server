...
setNotify(toast.error(error))      
  ...
  return (
    ...
    <div className='flex flex-col p-3 text-right '>
    {
       ...
       <textarea id="message" rows="4" className="bg-gray-800  rounded-sm w-full border-2 border-gray-800"  value={text} onChange={(e)=>setText(e.target.value)}></textarea>
     </div>
 ...
 <div className=" w-full h-60 overflow-y-auto flex flex-col flex-grow bg-gray-800 mr-3 mt-3 rounded-sm">
              {
                notes?.data?.length != 0 ? (
                  ...
                  <div key={i} className='border-2  border-gray-800  w-full mb-2'>
        <h3 className='bg-gray-950 p-1'>المتابعة رقم[{i+1}]</h3>
        <p>{x?.note}</p>
        <p className='bg-gray-950 p-1'>تاريخ المتابعة : {new Date(x?.createdAt).toISOString().replace(/T/, ' ').replace(/\..+/, '')}</p>
    </div>
            
                  ))
                ) : (
                  <div className='border-2  border-gray-800  w-full mb-2'>
        <h3 className='bg-gray-950 p-1'>لا يوجد اي متابعات سابقة</h3>

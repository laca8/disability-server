...
<h1 className='p-2 bg-gray-950 rounded-md text-white font-bold mb-2 w-1/2  mt-32'>الحالة التعليمية</h1>
<div className='flex flex-row w-1/2 '>
    <input value={title} onChange={(e)=>setTitle(e.target.value)} type='text' placeholder='اضافة حالة تعليمية جديدة' className='border-2 border-gray-100 p-2 bg-gray-800 w-full'/>
    <button className='bg-cyan-700 p-1 text-white cursor-pointer' onClick={()=>handleAdd()}>اضافة</button>
</div>
...

{
  data ? data && data?.map((x,i)=>(
        <div key={i} className='p-1 flex flex-row justify-between rounded-md w-full bg-black text-white mt-2 '>
            <span>{x.title}</span>

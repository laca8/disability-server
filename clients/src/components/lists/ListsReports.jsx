...
<div className="w-full h-72 overflow-y-auto flex flex-col flex-grow shadow-md sm:rounded-lg font-bold ">
{
      ...
      </div>
  {
    ...
    <p className='bg-gray-800 text-right p-1 text-white rounded-sm mt-2 mb-2'>لا يوجد بيانات تتماشي مع عملية البحث حاول مرة أخرى</p>
     : (
        ...
        <tbody className='bg-zinc-100 text-gray-800 '>
            
            {
                reports?.data && reports?.data?.map((x,i)=>(
                    <tr key={i} className=" border-b-2 border-black   hover:bg-gray-400 hover:text-black">
              ...
              {x?.connName}
                ...
                <td className="px-6 py-4 text-right ">
                    ...
                    <button  className="font-medium text-white bg-cyan-700 p-1 m-1" onClick={()=>{clickToShow(x?._id)}}>عرض</button>
                </td>
